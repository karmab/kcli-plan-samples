K8S="{{ k8s_version }}"
[ "$K8S" == "latest" ] && K8S=$(curl -s https://api.github.com/repos/kubernetes/kubernetes/releases|grep tag_name|sort -V | tail -1 | awk -F':' '{print $2}' | sed 's/,//' | xargs | awk -F'-' '{print $1}' | sed 's/v//')
